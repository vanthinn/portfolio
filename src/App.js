...
function App() {
  ...
  let animate = useRef('')
  theme === 'dark-mode' ? particle.current = Arrayparticles[5].options : particle.current = Arrayparticles[1].options
  const handleClickTheme = () => {
    theme === 'dark-mode' ? SetTheme('light-mode'): SetTheme('dark-mode')
  }

  const handleMenu = () => {
    animate.current ==='animateOn linear 1s' ? animate.current = 'animateOff linear 1.2s':animate.current = 'animateOn linear 1s'
